...
public:
    IgorMunizNascimento() {
        nome = "Igor Muniz Nascimento";
        descricao =
            "Engenheiro de software focado em soluções eficientes, inovadoras e de alta qualidade.\n"
            "Especialista em programação orientada a objetos, arquitetura de sistemas escaláveis e robustos.\n"
            "Experiência em desenvolvimento web e automações em Python, com scripts de segurança e testes de invasão.\n"
            "Stacks e tecnologias: Python, PHP, Java, TypeScript, JavaScript; frameworks: Django, Flask, Laravel, Spring Boot.\n"
            "Paixão por Inteligência Artificial, trabalhando com NLP, LLMs, visão computacional e modelos preditivos.\n"
            "Contribuo em projetos colaborativos como o Community/Global-Campus-Teachers, visando melhorar a educação global.";

        portfolioURL = "https://imnascimento.github.io/Portifolio/";
    }

    void exibirPerfil() {
        cout << "Olá, eu sou " << nome << ".\n\n";
        cout << descricao << endl << endl;
    }

    void visitarPortfolio() {
        cout << "Abrindo portfólio no navegador..." << endl;
        string command = "start " + portfolioURL;
#ifdef _WIN32
        system(command.c_str());
#elif __APPLE__
        command = "open " + portfolioURL;
        system(command.c_str());
#else
        command = "xdg-open " + portfolioURL;
        system(command.c_str());
#endif
    }
};
...
int main() {
    IgorMunizNascimento igor;

    igor.exibirPerfil();

    char escolha;
    cout << "Deseja visitar o portfólio? (s/n): ";
    cin >> escolha;

    if (escolha == 's' || escolha == 'S') {
        igor.visitarPortfolio();
    } else {
        cout << "Obrigado por conhecer meu perfil!" << endl;
    }
...
class Company {
    ...
    public function __construct(string $name, string $description, string $startDate, ?string $endDate, string $status) {
        $this->name = $name;
    ...
    public function getSummary(): string {
        $period = $this->endDate ? "$this->startDate até $this->endDate" : "$this->startDate até o momento";
        return <<<EOD
        Empresa: {$this->name}
        Período: {$period}
        Status: {$this->status}
...
class BusinessProfile {
    ...
    public function showCompaniesSummary(): void {
        foreach ($this->companies as $company) {
            echo $company->getSummary() . "\n-------------------------\n";
        }
    }
}
...
$profile->addCompany(new Company(
    'SophiaLabs',
    'Desenvolvimento de softwares inovadores e estratégias de mercado orientadas a dados.',
    '02/2024',
    null,
    'Ativa'
));
...
$profile->addCompany(new Company(
    'NArmament',
    'Empresa de manutenção de equipamentos bélicos e recreativos, e-commerce e sistemas internos em PHP.',
    '01/2018',
    '25/01/2023',
    'Vendida'
));
$profile->showCompaniesSummary();
```
...
const IgorMunizNascimento: DeveloperProfile = {
    name: 'Igor Muniz Nascimento',
    knowledgeArea: 'Software Engineer and Developer',

    languages: [
        { name: 'PHP', type: SkillType.LANGUAGE },
        ...
        { name: 'C++', type: SkillType.LANGUAGE },
        { name: 'C', type: SkillType.LANGUAGE },
        { name: 'HTML5', type: SkillType.LANGUAGE },
        { name: 'CSS3', type: SkillType.LANGUAGE },
        { name: 'Shell Script', type: SkillType.LANGUAGE },
    ],

    frameworks: [
        { name: 'Django', type: SkillType.FRAMEWORK },
        { name: 'Flask', type: SkillType.FRAMEWORK },
        { name: 'Laravel', type: SkillType.FRAMEWORK },
        { name: 'Angular', type: SkillType.FRAMEWORK },
        { name: 'React', type: SkillType.FRAMEWORK },
        { name: 'Next.js', type: SkillType.FRAMEWORK },
        { name: 'Spring Boot', type: SkillType.FRAMEWORK },
        { name: 'Bootstrap', type: SkillType.FRAMEWORK },
        { name: 'Tailwind CSS', type: SkillType.FRAMEWORK },
        { name: 'FastApi', type: SkillType.FRAMEWORK },
    ],

    databases: [
        { name: 'MySQL', type: SkillType.DATABASE },
        { name: 'PostgreSQL', type: SkillType.DATABASE },
        { name: 'SQLite', type: SkillType.DATABASE },
        { name: 'MariaDB', type: SkillType.DATABASE },
        { name: 'Redis', type: SkillType.DATABASE },
    ],

    tools: [
        { name: 'Git', type: SkillType.TOOL },
        { name: 'Docker', type: SkillType.TOOL },
        { name: 'CI/CD', type: SkillType.TOOL },
        { name: 'NGINX', type: SkillType.TOOL },
        { name: 'DigitalOcean', type: SkillType.TOOL },
        { name: 'Swagger', type: SkillType.TOOL },
        { name: 'Postman', type: SkillType.TOOL },
        { name: 'Jupyter Notebook', type: SkillType.TOOL },
        { name: 'Eclipse', type: SkillType.TOOL },
        { name: 'VSCode', type: SkillType.TOOL },
        { name: 'Vim', type: SkillType.TOOL },
    ],

    operatingSystems: [
        { name: 'Linux', type: SkillType.OS },
        { name: 'Ubuntu', type: SkillType.OS },
        { name: 'Debian', type: SkillType.OS },
        { name: 'CentOS', type: SkillType.OS },
        { name: 'Kali Linux', type: SkillType.OS },
        { name: 'Linux Mint', type: SkillType.OS },
        { name: 'Windows', type: SkillType.OS },
        { name: 'MacOS', type: SkillType.OS },
    ],

    hardware: [
        { name: 'Arduino', type: SkillType.HARDWARE },
        { name: 'Raspberry Pi', type: SkillType.HARDWARE },
    ],

    softSkills: [
        { name: 'Trabalho em Equipe', type: SkillType.SOFT_SKILL },
        { name: 'Liderança', type: SkillType.SOFT_SKILL },
        { name: 'Proatividade', type: SkillType.SOFT_SKILL },
        { name: 'Resiliência', type: SkillType.SOFT_SKILL },
        { name: 'Inteligência emocional', type: SkillType.SOFT_SKILL },
        { name: 'Autonomia', type: SkillType.SOFT_SKILL },
    ],

    hardSkills: [
        { name: 'Object Oriented Programming', type: SkillType.HARD_SKILL },
        { name: 'Design Patterns', type: SkillType.HARD_SKILL },
        { name: 'Fundamentos Ethical Hacking', type: SkillType.HARD_SKILL },
        { name: 'Mikrotik Advanced', type: SkillType.HARD_SKILL },
        { name: 'Firewall Linux Iptables', type: SkillType.HARD_SKILL },
        { name: 'SEO', type: SkillType.HARD_SKILL },
        { name: 'Machine Learning & IA', type: SkillType.HARD_SKILL },
        { name: 'Ballistics Applied to Forensics', type: SkillType.HARD_SKILL },
        { name: 'Aeg Maintenance', type: SkillType.HARD_SKILL },
        { name: 'GBB Maintenance', type: SkillType.HARD_SKILL },
        { name: 'Airsoft Sniper Maintenance', type: SkillType.HARD_SKILL },
        { name: 'Firearms Dispatcher', type: SkillType.HARD_SKILL },
    ],
};

console.log(IgorMunizNascimento);
